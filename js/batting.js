// batting.js - playable batting mechanic
...
export default {
  ...
  playBall(){
    return new Promise((resolve)=>{
      ...
      const area = document.getElementById('match-area'); area.innerHTML = '';
      const card = document.createElement('div');card.className='card';
      const info = document.createElement('div');info.textContent='Press SPACE to time your shot. Green = perfect';
      ...
      let pos=0,dir=1;let running=true;
      function step(){ if(!running) return; pos+=dir*2; if(pos>100){pos=100;dir=-1} if(pos<0){pos=0;dir=1} mover.style.left = pos+'%'; requestAnimationFrame(step)}
      step();
      function onHit(){ running=false; document.removeEventListener('keydown',key); 
      ...
      function key(e){ if(e.code==='Space'){onHit()} }
